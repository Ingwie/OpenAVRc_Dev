 ...
 * Bit-Bang only as this is used in the Bootloader.
 * Originally written for Xmega hardware SPI.
 * Hardware SPI offers no benefit over Bitbang except the squareness of
 * the pulses. Hardware SPI can't do DMA and uses an extra pin even though
 * this is a unidirectional driver.
 * Choose Bit-Bang to reclaim the unused MISO pin.
...
#define ST756n_CMD_DISPLAY_INVERSE             0xA7
...
#define ST756n_CMD_CONTRAST_SET                0x81 // 2 Byte command
#define ST756n_CMD_SET_BOOSTER_LEVEL           0xF8 // 2 byte command
...
static void lcd_spi_tx(uint8_t c)
{
#if defined (BITBANGSPI)
#if defined (SPI9BIT)
...
static void lcdSendCmd(uint8_t c)
{
#if defined (BITBANGSPI)
  LCD_CS_ACTIVE;
...
static void lcdSetRefVolt(uint8_t val)
{
  ...
  if(val > ST756n_CONTRAST_MAX)
    val = ST756n_CONTRAST_MAX;
#endif

#if defined(BITBANGSPI)
  #if defined (LCD_EVO)
  lcdSendCmd(ST756n_CMD_CONTRAST_SET); lcdSendCmd(val);
#endif
#if defined (LCD_ST7567)
  lcdSendCmd(ST756n_CMD_CONTRAST_SET); lcdSendCmd(val);
#endif
...
#endif
#else
  // Interrupt driven SPI used lcdRefreshFast() but using interrupts were a big fail.
#endif
}
