...
ISR(TIMER1_COMPA_vect) // 2MHz pulse generation (BLOCKING ISR).
{
  ...
  if (IS_DSM2_SERIAL_PROTOCOL(s_current_protocol[0]) || *((uint16_t*)pulses2MHzRPtr) == 0) {
    if (!IS_DSM2_SERIAL_PROTOCOL(s_current_protocol[0])) {
      ...
      OCR5A = TCNT5 + SETUP_PULSES_DURATION;
      if (g_model.pulsePol) {
        TCCR1A = (TCCR1A | (1<<COM1B1)) & ~(1<<COM1B0); // Set idle level.
        TCCR5A = (TCCR5A | (1<<COM5A1)) & ~(1<<COM5A0); // Set idle level.
      }
      else {
        TCCR1A |= 3<<COM1B0;
        TCCR5A |= 3<<COM5A0;
      ...
      TCCR5C = 1<<FOC5A; // Strobe FOC5A.
      TCCR5A = (TCCR5A | (1<<COM5A0)) & ~(1<<COM5A1); // Toggle OC5A on next match.
    }
  ...
  if (s_current_protocol[0] != PROTO_NONE) {
    ...
    if (*(uint16_t*)(pulses2MHzRPtr + sizeof(uint16_t)) == 0) {
      ...
      OCR5A += 0xffff; // Prevent next compare match hence toggle.
    }
    else {
      OCR1B = *((uint16_t*) pulses2MHzRPtr);
      OCR5A += *((uint16_t*) pulses2MHzRPtr);
    }
  }
...
#if defined(DSM2_PPM) && defined(TX_CADDY)
