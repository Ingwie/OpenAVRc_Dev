...
// PORTH2 XCK2
  ...
  #define RF_XCK_OFF()              PORTH &= ~(OUT_H_RF_XCK)
	...
	#define RF_MOSI_ON                (PINH & (OUT_H_RF_MOSI))
	#define SUSPEND_RF_SPI()          (UCSR2B = 0) // Disable RX and TX Mspi mode
	#define WAKEUP_RF_SPI()           (UCSR2B = (1 << RXEN2) | (1 << TXEN2)) // Enable RX and TX Mspi mode
