 ...
 * - \link SD_partition SD_partition table support \endlink
 ...
 * - <tt>cd \<SD_dir_entry\></tt>\n
 *   Changes current working SD_dir_entry to \<SD_dir_entry\>.
 * - <tt>disk</tt>\n
 *   Shows card manufacturer, status, filesystem capacity and free storage space.
 * - <tt>init</tt>\n
 *   Reinitializes and reopens the memory card.
 * - <tt>ls</tt>\n
 *   Shows the content of the current SD_dir_entry.
 * - <tt>mkdir \<SD_dir_entry\></tt>\n
 *   Creates a SD_dir_entry called \<SD_dir_entry\>.
 ...
 *
 * <p>
 * When opening a SD_partition, filesystem, file or SD_dir_entry, a little amount
 ...
 * memory card. The other parts like SD_partition table and FAT support are
...
static uint8_t find_file_in_dir(struct fat_fs_struct* SD_filesystem, struct fat_dir_struct* SD_dir, const char* name, struct fat_dir_entry_struct* dir_entry);
static struct fat_file_struct* open_file_in_dir(struct fat_fs_struct* SD_filesystem, struct fat_dir_struct* SD_dir, const char* name);
static uint8_t print_disk_info(const struct fat_fs_struct* SD_filesystem);
  ...
  while(1)
    {
      ...
      /* open first SD_partition */
      struct partition_struct* SD_partition = partition_open(sd_raw_read,
      ...
      if(!SD_partition)
        {
          /* If the SD_partition did not open, assume the storage device
          ...
          if(!SD_partition)
            {
#if DEBUG
              uart_puts_p(PSTR("opening SD_partition failed\n"));
#endif
              continue;
            }
        }
      ...
      /* open file system */
      struct fat_fs_struct* SD_filesystem = fat_open(SD_partition);
      if(!SD_filesystem)
        {
#if DEBUG
      ...
      /* open root SD_dir_entry */
      struct fat_dir_entry_struct SD_dir_entry;
      fat_get_dir_entry_of_path(SD_filesystem, "/", &SD_dir_entry);

      struct fat_dir_struct* SD_dir = fat_open_dir(SD_filesystem, &SD_dir_entry);
      if(!SD_dir)
        {
#if DEBUG
          uart_puts_p(PSTR("opening root SD_dir_entry failed\n"));
#endif
          continue;
        }

      /* print some card information as a boot message */
      print_disk_info(SD_filesystem);
      ...
      while(1)
        {
          ...
          if(strcmp_P(command, PSTR("init")) == 0)
          ...
          else if(strncmp_P(command, PSTR("cd "), 3) == 0)
            {
              ...
              if(find_file_in_dir(SD_filesystem, SD_dir, command, &subdir_entry))
                {
                  struct fat_dir_struct* dd_new = fat_open_dir(SD_filesystem, &subdir_entry);
                  if(dd_new)
                    {
              ...
              uart_puts_p(PSTR("SD_dir_entry not found: "));
              uart_puts(command);
              uart_putc('\n');
            }
          else if(strcmp_P(command, PSTR("ls")) == 0)
            {
              /* print SD_dir_entry listing */
              struct fat_dir_entry_struct dir_entry;
              while(fat_read_dir(SD_dir, &dir_entry))
                {
          ...
          else if(strncmp_P(command, PSTR("cat "), 4) == 0)
            {
              ...
              /* search file in current SD_dir_entry and open it */
              struct fat_file_struct* fd = open_file_in_dir(SD_filesystem, SD_dir, command);
              if(!fd)
                {
            ...
            {
              if(!print_disk_info(SD_filesystem))
          ...
          else if(strncmp_P(command, PSTR("rm "), 3) == 0)
            {
              ...
              if(find_file_in_dir(SD_filesystem, SD_dir, command, &file_entry))
                {
                  if(fat_delete_file(SD_filesystem, &file_entry))
                    continue;
                }
          ...
          else if(strncmp_P(command, PSTR("touch "), 6) == 0)
            {
              ...
              struct fat_dir_entry_struct file_entry;
              if(!fat_create_file(SD_dir, command, &file_entry))
                {
          ...
          else if(strncmp_P(command, PSTR("mv "), 3) == 0)
            {
              ...
              if(find_file_in_dir(SD_filesystem, SD_dir, command, &file_entry))
                {
                  if(fat_move_file(SD_filesystem, &file_entry, SD_dir, target))
                    continue;
                }
          ...
          else if(strncmp_P(command, PSTR("mkdir "), 6) == 0)
            {
              ...
              struct fat_dir_entry_struct dir_entry;
              if(!fat_create_dir(SD_dir, command, &dir_entry))
                {
                  uart_puts_p(PSTR("error creating SD_dir_entry: "));
                  uart_puts(command);
                  uart_putc('\n');
                }
            }
...
uint8_t find_file_in_dir(struct fat_fs_struct* SD_filesystem, struct fat_dir_struct* SD_dir, const char* name, struct fat_dir_entry_struct* dir_entry)
{
  while(fat_read_dir(SD_dir, dir_entry))
    {
      if(strcmp(dir_entry->long_name, name) == 0)
        {
          fat_reset_dir(SD_dir);
          return 1;
        }
    }
...
struct fat_file_struct* open_file_in_dir(struct fat_fs_struct* SD_filesystem, struct fat_dir_struct* SD_dir, const char* name)
{
  struct fat_dir_entry_struct file_entry;
  if(!find_file_in_dir(SD_filesystem, SD_dir, name, &file_entry))
    return 0;

  return fat_open_file(SD_filesystem, &file_entry);
}

uint8_t print_disk_info(const struct fat_fs_struct* SD_filesystem)
{
  if(!SD_filesystem)
    return 0;
  ...
  uart_puts_p(PSTR("manuf:  0x"));
  uart_putc_hex(disk_info.manufacturer);
  uart_putc('\n');
  uart_puts_p(PSTR("oem:    "));
  uart_puts((char*) disk_info.oem);
  uart_putc('\n');
  uart_puts_p(PSTR("prod:   "));
  uart_puts((char*) disk_info.product);
  uart_putc('\n');
  uart_puts_p(PSTR("rev:    "));
  uart_putc_hex(disk_info.revision);
  uart_putc('\n');
  uart_puts_p(PSTR("serial: 0x"));
  uart_putdw_hex(disk_info.serial);
  uart_putc('\n');
  uart_puts_p(PSTR("date:   "));
  uart_putw_dec(disk_info.manufacturing_month);
  uart_putc('/');
  uart_putw_dec(disk_info.manufacturing_year);
  uart_putc('\n');
  uart_puts_p(PSTR("size:   "));
  uart_putdw_dec(disk_info.capacity / 1024 / 1024);
  uart_puts_p(PSTR("MB\n"));
  uart_puts_p(PSTR("copy:   "));
  uart_putw_dec(disk_info.flag_copy);
  uart_putc('\n');
  uart_puts_p(PSTR("wr.pr.: "));
  uart_putw_dec(disk_info.flag_write_protect_temp);
  uart_putc('/');
  uart_putw_dec(disk_info.flag_write_protect);
  uart_putc('\n');
  uart_puts_p(PSTR("format: "));
  uart_putw_dec(disk_info.format);
  uart_putc('\n');
  uart_puts_p(PSTR("free:   "));
  uart_putdw_dec(fat_get_fs_free(SD_filesystem));
  uart_putc('/');
  uart_putdw_dec(fat_get_fs_size(SD_filesystem));
  uart_putc('\n');
