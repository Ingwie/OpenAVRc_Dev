...
//uint8_t Usart0TxBuffer[USART0_TX_PACKET_SIZE];   // Ditto for transmit buffer
...
void processSerialData(uint8_t data)
{
  ...
  if (data == START_STOP) {
    ...
    switch (dataState) {
      case STATE_DATA_XOR:
        Usart0RxBuffer[numPktBytes++] = data ^ STUFF_MASK;
        dataState = STATE_DATA_IN_FRAME;
        break;
      ...
      case STATE_DATA_IN_FRAME:
        if (data == BYTESTUFF)
          ...
          Usart0RxBuffer[numPktBytes++] = data;
        break;
    }
  }

  if (numPktBytes == USART0_RX_PACKET_SIZE) {
