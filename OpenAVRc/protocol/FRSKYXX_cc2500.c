...
const static RfOptionSettingsvarstruct RfOpt_FrskyXX_Ser[] PROGMEM = {
...
const pm_char STR_SUBTYPE_FrskyXX[] PROGMEM =     " FCC"" LBT";

bool FrskyXX_format_EU = 1; //0 US , 1 EU
...
static void FrskyXX_initialize_data(uint8_t adr)
{
...
static void set_start(uint8_t ch)
{
  ...
  /*CC2500_WriteReg(CC2500_23_FSCAL3, calDataX[ch][0]);
  CC2500_WriteReg(CC2500_24_FSCAL2, calDataX[ch][1]);
  CC2500_WriteReg(CC2500_25_FSCAL1, calDataX[ch][2]);*/
...
static void FrskyXX_build_bind_packet()
{
 /* packet[0] = g_model.rfSubType ? 0x20 : 0x1D;// LBT (EU) or  FCC (US)
  packet[1] = 0x03;
  packet[2] = 0x01;
...
static void FrskyXX_data_frame()
{
  ...
  packet[0] = g_model.rfSubType ? 0x20 : 0x1D;
...
static void FrskyXX_initialize(uint8_t bind)
{
  PROTO_Stop_Callback();
  CC2500_Reset();
  packet_size = g_model.rfSubType ? 33 : 30;
  channel_offset = 0;
  channr = 0;
  chanskip = 0;
  ctr = 0;
...
#define random() rand()
  ...
  if (bind) {
    ...
    FrskyXX_initialize_data(1);
    X_state = 0;
    PROTO_Start_Callback(25000U *2, FrskyXX_bind_cb);

  } else {
    //PROTOCOL_SetBindState(0);
    FrskyXX_initialize_data(0);
    channr = 0;
    X_state = FrskyXX_DATA1;
    PROTO_Start_Callback(25000U *2, FrskyXX_cb);
  }
}

const void *FRSKYXX_Cmds(enum ProtoCmds cmd)
{
  switch(cmd) {
  case PROTOCMD_INIT:
    FrskyXX_initialize(0);
    return 0;
  case PROTOCMD_BIND:
    FrskyXX_initialize(1);
    return 0;
  case PROTOCMD_RESET:
    PROTO_Stop_Callback();
    CC2500_Reset();
    ...
    case PROTOCMD_GETOPTIONS:
          SetRfOptionSettings(pgm_get_far_address(RfOpt_FrskyXX_Ser),
                        STR_SUBTYPE_FrskyXX,       //Sub proto
