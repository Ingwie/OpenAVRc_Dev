...
const static int8_t RfOpt_DDSM_Ser[] PROGMEM = {
...
const uint8_t PROGMEM zzch_map_progmem[][14] = {
...
static void DSM_cyrf_config()
{
  ...
  {
		CYRF_WriteRegister(pgm_read_byte_far(init_vals++), pgm_read_byte_far(init_vals++));
  }
...
static void DSM_cyrf_configdata()
{
  ...
  {
		CYRF_WriteRegister(pgm_read_byte_far(data_vals++), pgm_read_byte_far(data_vals++));
  }
}
...
static void DSM_set_sop_data_crc()
{
	...
	uint8_t pn_row = get_pn_row(channel_used[channel_used_no]);
	uint8_t code[16];
	...
	CYRF_ConfigRFChannel(channel_used[channel_used_no]);
	channel_used_no++;
	if(sub_protocol == DSMX_11 || sub_protocol == DSMX_22)
		channel_used_no %=23;
	else
...
const static uint8_t ZZ_DDSMInitSequence[] PROGMEM = {
};

static void DDSM_init()
{
}


static void DDSM_send_data_packet()
{
}

static void DDSM_send_bind_packet()
{
}

static uint16_t DDSM_bind_cb()
{
...
static void DDSM_initialize(uint8_t bind)
{
  DDSM_init();
  if (bind) {
  PROTO_Start_Callback( DDSM_bind_cb);
  } else {
  PROTO_Start_Callback( DDSM_cb);
  }
}

const void *DDSM_Cmds(enum ProtoCmds cmd)
{
  switch(cmd) {
  case PROTOCMD_INIT:
    DDSM_initialize(0);
  ...
  case PROTOCMD_GETOPTIONS:
          SetRfOptionSettings(pgm_get_far_address(RfOpt_DDSM_Ser),
                        STR_DUMMY,      //Sub proto
