...
const pm_char J6PRO_STR_NUMCH[] PROGMEM = INDENT"N Ch"; // TODO MAKE GLOBAL SHARED
const pm_char J6PRO_STR_STATICID[] PROGMEM = INDENT"Static ID";
#endif

#define J6PRO_NUM_WAIT_LOOPS (100 / 5) //each loop is ~5us.  Do not wait more than 100us

#define DEVO_AUTOBIND (g_model.rfOptionBool1)
#define USE_FIXED_ID  (g_model.rfOptionBool2)

const uint8_t J6PRO_data_code[] PROGMEM = { 0x02, 0xf9, 0x93, 0x97, 0x02, 0xfa, 0x5c, 0xe3,
...
void J6PRO_build_data_packet() {
  ...
  for(uint8_t upper=0; upper<3; upper++)
  {
    ...
    {
      int16_t value = channelOutputs[(upper << 2) | lower]  /2;
...
void J6PRO_cyrf_datainit() {
  ...
  uint8_t sopidx = (0xff
      & (cyrfmfg_id[0] + cyrfmfg_id[1] + cyrfmfg_id[2] + cyrfmfg_id[3]
          - cyrfmfg_id[5])) % 19;
  uint16_t crc =
      (0xff & (cyrfmfg_id[1] - cyrfmfg_id[4] + cyrfmfg_id[5]))
          | ((0xff
              & (cyrfmfg_id[2] + cyrfmfg_id[3] - cyrfmfg_id[4] + cyrfmfg_id[5]))
              << 8);
...
uint16_t J6PRO_cb() {
  ...
  switch (rfState8) {
    ...
    //return 3750
    case J6PRO_CHAN_4:
    CYRF_ConfigRFChannel(radio_ch[rfState8 - J6PRO_CHAN_1]);
    CYRF_SetTxRxMode(TX_EN);
...
void J6PRO_Init(uint8_t bind) {
  CYRF_Reset();
  J6PRO_cyrf_init();
  uint_farptr_t dataadr = pgm_get_far_address(J6PRO_data_code);
  uint8_t codedata[16];
...
const void *J6PRO_Cmds(enum ProtoCmds cmd) {
  ...
  case PROTOCMD_GETOPTIONS:
    SetRfOptionSettings(pgm_get_far_address(RfOpt_DDEVO_Ser), STR_DUMMY, //Sub proto
