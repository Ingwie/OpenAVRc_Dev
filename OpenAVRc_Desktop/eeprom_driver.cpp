...
{
  eeprom[eeprom_pointer] = *eeprom_buffer_data;
}
...
void isr_ee_ready()
{
  if (--eeprom_buffer_size > 0) {
   ...
   // TRACE("ISR 'isr_ee_ready' Cleared : Eeprom is saved");
   // EECR &= ~(1<<EERIE);
  }
}
...
void eepromWriteBlock(uint8_t * i_pointer_ram, uint16_t i_pointer_eeprom, size_t size)
{
  ...
  if (s_sync_write) {
    while (eeprom_buffer_size > 0) isr_ee_ready(); //Simulate ISR
  }
}
...
void RlcFile::flush()
{
  while (eeprom_buffer_size > 0) isr_ee_ready(); //Simulate ISR in Simu mode
