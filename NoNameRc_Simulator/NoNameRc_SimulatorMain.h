...
#ifndef NoNameRc_SIMULATORMAIN_H
#define NoNameRc_SIMULATORMAIN_H

//(*Headers(NoNameRc_SimulatorFrame)
#include <wx/menu.h>
...
class NoNameRc_SimulatorFrame: public wxFrame
{
public:

    NoNameRc_SimulatorFrame(wxWindow* parent,wxWindowID id = -1);
    virtual ~NoNameRc_SimulatorFrame();
    const void DrawWxSimuLcd();
    void OnKey(wxKeyEvent& Event);

private:
    //(*Handlers(NoNameRc_SimulatorFrame)
    ...
    //(*Identifiers(NoNameRc_SimulatorFrame)
    ...
    //(*Variables(NoNameRc_SimulatorFrame)
    ...
    //(*Declarations(NoNameRc_SimulatorFrame)
    wxPanel* BpId2;
    wxPanel* BPg;
    wxTimer Timer10ms;
...
#endif // NoNameRc_SIMULATORMAIN_H
